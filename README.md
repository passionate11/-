...
- 设置页原生细节再打磨：整体底色更柔和，侧栏品牌区、节奏摘要、导航图标和右侧页面标题区进一步收紧，主卡片初始阴影继续降低，减少自绘面板感。
- 设置页 0.1.45 继续降噪：右侧内容区上移并加宽，主卡片增高但边框/阴影更轻，侧栏导航行更窄更像 macOS source list，时间输入框收成小号控件。
- 设置页 0.1.48 视觉精修：侧栏导航收进成组 source list，品牌徽章和节奏摘要更轻，概览页重排为状态带、双计时、信息带和轻工具条，继续减少灰框按钮和硬卡片感。
- 设置页 0.1.49 继续收敛：左侧品牌和节奏摘要再缩小，导航行更像 macOS source list，主卡片增高但边线/阴影更低，概览增加状态胶囊、图标底座和更干净的时间输入。
...
- 自动化关键词目录覆盖会议协作、视频游戏、录制面试三类高频场景：Webex/Skype、Movist/Plex、Battle.net/Riot、ScreenFlow/Camtasia、录屏、答辩和考试等默认词会随内置目录更新；如果用户已经自定义关键词，不会被覆盖。
...
- 自动化设置页可直接复制当前策略诊断，不用回到菜单栏，也方便把命中原因、最近动作和建议下一步一起贴到反馈里。
- 支持自定义轻打扰应用白名单。
...
- 菜单栏支持复制安装更新说明，可把下载 zip、sha256 校验、覆盖安装、确认版本和反馈问题步骤一次性复制出来。
...
菜单栏里的 `排查中心` -> `复制安装更新说明` 会把当前版本、安装位置、下载页、建议 zip、SHA256 校验文件、覆盖安装和反馈包入口复制到剪贴板，适合发给别人或自己留存。
...
2. 同页可下载 `songyixia-<version>-<build>.zip.sha256`，需要转发或公开分发时可先用 `shasum -a 256 -c ...` 校验。
3. 退出正在运行的松一下，解压得到 `松一下.app`，拖入 `/Applications` 覆盖旧版本。
4. 重新打开菜单栏 `关于 松一下...` 确认版本，或用 `检查更新...` 查看 GitHub 最新版本；有新版时可直接打开 zip 下载。
5. 反馈问题时先复制问题反馈包，再点 `反馈问题...` 打开预填好的 Issue。
...
脚本只读地检查自动化关键词弹窗的追加语义、当前应用/当前日程按钮、会议协作/视频游戏/录制面试推荐模板、大小写去重、恢复默认和保存边界。它不会启动 App，也不会触发全屏休息页，适合低打扰验证“追加推荐词不会覆盖用户已有配置”。
...
脚本只读地检查设置窗口的 944px 宽面板、左侧原生侧栏、右侧 664px 内容区、概览/眼睛/站立/显示/自动化/统计页的关键布局、主题降噪和文档记录。它不会启动 App，也不会弹出休息页，适合在少打扰的情况下守住设置页观感。
...
- 自动化设置页诊断入口：策略结论旁边可直接复制自动化诊断，配置、判断和反馈能在同一个页面完成。
...
- 自动化推荐模板：关键词弹窗可一键追加会议协作、视频游戏、录制面试三类常见策略关键词，只追加并去重，不覆盖已有配置。
- 自动化关键词目录：内置目录补充 Webex/Skype、Movist/Plex、Battle.net/Riot、ScreenFlow/Camtasia，以及周会、例会、复盘、录屏、答辩、考试等日程关键词；旧默认目录会自动升级，手动改过的配置不覆盖。
...
- 安装更新说明：菜单栏可复制当前版本、下载页、zip/sha256 文件、覆盖安装、版本确认和反馈步骤。
...
- 设置页 0.1.45 继续降噪：右侧内容区上移并加宽，主卡片增高但降低边框和阴影，侧栏导航行更窄更像 source list，时间输入框和分钟/秒单位标签收成更轻的 macOS 控件节奏。
- 设置页 0.1.48 视觉精修：侧栏导航收进成组 source list，品牌徽章和节奏摘要更轻，主卡片加高到 332px 且阴影降到更低，概览页新增信息带和主题强调线，让默认首页不再像灰色调试面板。
- 设置页 0.1.49 继续收敛：品牌徽章缩到 34px，节奏摘要缩到 184x54，source list 行距收紧到 28px，主卡片加高到 340px 且初始阴影降到 0.002，概览页加入状态胶囊、图标底座和居中的时间输入，让首页更像 macOS 设置面板。
- 自动化关键词配置检查：新增只读脚本覆盖当前应用/当前日程追加、三类推荐模板、分隔符清洗、大小写去重、恢复默认和不覆盖已有配置，并接入 CI、preflight、路线图和发布证据包。
- 自动化页 0.1.46 闭环补强：策略结论旁边新增复制诊断按钮，复制内容和菜单栏自动化诊断一致，方便配置完立刻确认命中原因或贴到反馈里。
- 设置页原生感微调：侧栏点击层改成不绘制的 `ERSettingsSidebarButton`，选中行改用更安静的 source list 背景，概览快捷操作条弱化为轻 toolbar。
- 设置页原生细节再打磨：默认背景、侧栏摘要、导航图标、标题徽章和强调线继续收窄变轻，主卡片初始阴影降到更接近 macOS 设置页的安静层级。
...
- 计划：继续把自动化页从“能诊断”推进到“更容易配置”，下一步重点看默认关键词、旧默认目录升级和推荐模板是否覆盖常见会议/视频/游戏/录制面试场景。
- 验收：设置页和问题反馈包都能直接回答“为什么这次只通知/自动暂停/正常弹窗”；关键词编辑弹窗能说明优先级、保留默认恢复并提供推荐模板；自动化关键词目录覆盖 Webex/Skype、Movist/Plex、Battle.net/Riot、ScreenFlow/Camtasia、录屏、答辩和考试；`automation_policy_readiness.sh --strict` 通过。
