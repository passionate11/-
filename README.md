...
- 计划：继续把自动化页从“能诊断”推进到“更容易配置”，下一步重点看默认关键词是否足够合理，以及推荐模板是否覆盖常见会议/视频/游戏/录制面试场景。
- 验收：设置页和问题反馈包都能直接回答“为什么这次只通知/自动暂停/正常弹窗”；关键词编辑弹窗能说明优先级、保留默认恢复并提供推荐模板；`automation_policy_readiness.sh --strict` 通过。
