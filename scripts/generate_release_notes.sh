...
1. 下载 \`${ARCHIVE_NAME}\`，同页可下载 \`${CHECKSUM_NAME}\` 做 SHA256 校验。
2. 退出正在运行的松一下，解压 zip 得到 \`松一下.app\`。
3. 把 \`松一下.app\` 拖入 \`/Applications\` 覆盖旧版本；如果系统询问，选择替换。
4. 重新打开松一下，在菜单栏「关于 松一下...」确认版本，或用「检查更新...」查看当前版本。
5. 如果 macOS 提示来自未验证开发者，可以在 Finder 里右键打开，或到系统设置里允许打开。
