...
bash -n scripts/automation_keyword_config_readiness.sh
...
bash -n scripts/swiftui_parity_plan.sh
...
check_contains "$SOURCE_CONTENT" "overviewSoftAccentViews" "settings overview soft accent guard"
check_contains "$SOURCE_CONTENT" "NSMakeRect(496, 15, 104, 24)" "settings overview status pill guard"
check_contains "$SOURCE_CONTENT" "NSMakeRect(14, 52, 24, 24)" "settings overview icon well guard"
...
check_contains "$SOURCE_CONTENT" "ERColor(0.968, 0.971, 0.976, 1)" "settings soft content background guard"
check_contains "$SOURCE_CONTENT" "sidebarSelectionViews" "settings sidebar selection guard"
check_contains "$SOURCE_CONTENT" "sidebarNavGroupView" "settings sidebar grouped nav guard"
check_contains "$SOURCE_CONTENT" "sidebarBrandBadge" "settings sidebar brand badge guard"
check_contains "$SOURCE_CONTENT" "NSMakeRect(28, 522, 34, 34)" "settings compact brand badge guard"
check_contains "$SOURCE_CONTENT" "NSMakeRect(24, 440, 184, 54)" "settings soft summary card guard"
check_contains "$SOURCE_CONTENT" "NSMakeRect(6, 160 - index * 32, 196, 28)" "settings compact sidebar row guard"
check_contains "$SOURCE_CONTENT" "configurationWithPointSize:12" "settings compact sidebar icon guard"
check_contains "$SOURCE_CONTENT" "ERSettingsSidebarButton" "settings sidebar non-drawing button guard"
check_contains "$SOURCE_CONTENT" "- (void)drawRect:(NSRect)dirtyRect" "settings sidebar no-draw guard"
...
check_contains "$SOURCE_CONTENT" "unemphasizedSelectedContentBackgroundColor" "settings quiet selected row guard"
check_contains "$SOURCE_CONTENT" "selectedIconColor" "settings selected icon accent guard"
check_contains "$SOURCE_CONTENT" "pageIconBadgeViews" "settings page icon badge guard"
check_contains "$SOURCE_CONTENT" "NSMakeRect(6, 408, 32, 32)" "settings compact page icon guard"
...
check_contains "$SOURCE_CONTENT" "NSMakeRect(50, 412, 24, 2)" "settings quiet page accent guard"
check_contains "$SOURCE_CONTENT" "card.layer.shadowOpacity = 0.002" "settings initial card shadow guard"
check_contains "$SOURCE_CONTENT" "field.controlSize = NSControlSizeSmall" "settings small time input guard"
check_contains "$SOURCE_CONTENT" "field.font = [NSFont monospacedDigitSystemFontOfSize:13.5 weight:NSFontWeightRegular]" "settings centered time input typography guard"
...
check_contains "$SOURCE_CONTENT" "ERSettingsAccentColor" "settings accent contrast guard"
...
check_contains "$README_CONTENT" "设置页原生细节再打磨" "settings native detail polish docs"
...
check_contains "$SOURCE_CONTENT" "overviewInsightBand" "settings overview insight band guard"
check_contains "$SOURCE_CONTENT" "overviewAccentViews" "settings overview accent guard"
check_contains "$SOURCE_CONTENT" "shell.layer.borderWidth = settingsDarkStyle ? 0.5 : 0" "settings overview toolbar action guard"
...
check_contains "$SOURCE_CONTENT" "建议下载" "install guide archive name"
check_contains "$SOURCE_CONTENT" "校验文件" "install guide checksum file"
check_contains "$SOURCE_CONTENT" "shasum -a 256 -c" "install guide checksum command"
check_contains "$SOURCE_CONTENT" "反馈包链接" "install guide issue bundle link"
...
check_contains "$(cat scripts/capture_release_evidence.sh)" "automation_keyword_config_readiness" "release evidence automation keyword config capture"
check_contains "$(cat scripts/capture_release_evidence.sh)" "manifest.json" "release evidence json manifest"
check_contains "$(cat scripts/capture_release_evidence.sh)" "evidence-checklist.md" "release evidence checklist"
check_contains "$(cat scripts/capture_release_evidence.sh)" "archiveSha256" "release evidence sha manifest"
check_contains "$(cat scripts/release_evidence_readiness.sh)" "release evidence assessed" "release evidence readiness summary"
check_contains "$(cat scripts/release_evidence_readiness.sh)" "machine-readable manifest" "release evidence manifest readiness"
check_contains "$(cat scripts/release_evidence_readiness.sh)" "human checklist" "release evidence checklist readiness"
...
check_contains "$SOURCE_CONTENT" "nextActionPlan=1" "roadmap next action plan marker"
check_contains "$SOURCE_CONTENT" "P1-product-polish,P2-automation-fit,P3-release-record" "roadmap next action priorities"
check_contains "$(cat scripts/roadmap_status.sh)" "next action plan" "roadmap next action readiness"
...
check_contains "$(cat scripts/generate_release_notes.sh)" "可选校验" "release notes optional checksum"
check_contains "$(cat scripts/generate_release_notes.sh)" "退出正在运行的松一下" "release notes quit old app step"
...
check_contains "$(cat scripts/swiftui_migration_readiness.sh)" "SwiftUI parity matrix" "SwiftUI migration matrix source"
check_contains "$(cat scripts/swiftui_migration_readiness.sh)" "Parity matrix features" "SwiftUI migration matrix count"
check_contains "$(cat scripts/swiftui_parity_plan.sh)" "SwiftUI parity plan assessed" "SwiftUI parity plan summary"
check_contains "$(cat scripts/swiftui_parity_plan.sh)" "settings-contract-foundation" "SwiftUI parity next phase"
check_contains "$(cat docs/swiftui-parity-matrix.json)" "\"migrationStatus\": \"prototype only\"" "SwiftUI parity matrix prototype status"
check_contains "$(cat docs/swiftui-parity-matrix.json)" "\"requiredBeforeSwitch\": true" "SwiftUI parity required-before-switch guard"
check_contains "$(cat docs/swiftui-migration-plan.json)" "\"migrationStatus\": \"prototype only\"" "SwiftUI migration plan prototype status"
check_contains "$(cat docs/swiftui-migration-plan.json)" "\"settings-contract-foundation\"" "SwiftUI migration plan settings phase"
check_contains "$(cat docs/swiftui-migration-plan.json)" "\"timers-rest-overlay\"" "SwiftUI migration plan timers phase"
check_contains "$(cat docs/swiftui-migration-plan.json)" "\"automation-recovery\"" "SwiftUI migration plan automation phase"
check_contains "$(cat docs/swiftui-migration-plan.json)" "\"stats-release-support\"" "SwiftUI migration plan release phase"
...
check_contains "$(cat scripts/settings_contract_readiness.sh)" "Machine-Readable Contract" "settings contract machine readable section"
check_contains "$(cat docs/settings-contract.json)" "\"storageModel\": \"per-key UserDefaults\"" "settings contract per-key storage doc"
check_contains "$(cat docs/settings-contract.json)" "\"quickSetupSeen\"" "settings contract onboarding key"
check_contains "$(cat docs/settings-contract.json)" "\"minimumDebugValue\": 10" "settings contract debug minimum"
check_contains "$README_CONTENT" "SwiftUI 迁移准备检查" "SwiftUI migration docs"
check_contains "$README_CONTENT" "docs/swiftui-parity-matrix.json" "SwiftUI parity matrix docs file"
check_contains "$README_CONTENT" "SwiftUI 迁移阶段计划" "SwiftUI migration phase plan docs"
check_contains "$README_CONTENT" "docs/swiftui-migration-plan.json" "SwiftUI migration plan docs file"
check_contains "$README_CONTENT" "设置合约准备检查" "settings contract docs"
check_contains "$README_CONTENT" "docs/settings-contract.json" "settings contract docs file"
...
check_contains "$(cat scripts/automation_keyword_config_readiness.sh)" "automation keyword config assessed" "automation keyword config readiness summary"
check_contains "$(cat scripts/release_evidence_readiness.sh)" "automation_keyword_config_readiness" "release evidence automation keyword config readiness"
check_contains "$SOURCE_CONTENT" "ERLegacyDefaultFocusAppTokens" "automation legacy focus catalog guard"
check_contains "$SOURCE_CONTENT" "ERUpgradedAutomationTokensIfUncustomized" "automation uncustomized upgrade guard"
check_contains "$SOURCE_CONTENT" "automation_keyword_catalog" "automation keyword catalog roadmap marker"
check_contains "$SOURCE_CONTENT" "com.cisco.webexmeetingsapp" "automation Webex keyword"
check_contains "$SOURCE_CONTENT" "com.skype.skype" "automation Skype keyword"
check_contains "$SOURCE_CONTENT" "com.movist.mac" "automation Movist keyword"
check_contains "$SOURCE_CONTENT" "com.plexapp.plex" "automation Plex keyword"
check_contains "$SOURCE_CONTENT" "battle.net" "automation Battle.net keyword"
check_contains "$SOURCE_CONTENT" "riot client" "automation Riot keyword"
check_contains "$SOURCE_CONTENT" "com.telestream.screenflow" "automation ScreenFlow keyword"
check_contains "$SOURCE_CONTENT" "com.techsmith.camtasia" "automation Camtasia keyword"
check_contains "$SOURCE_CONTENT" "录屏" "automation screen recording calendar keyword"
check_contains "$SOURCE_CONTENT" "答辩" "automation defense calendar keyword"
check_contains "$SOURCE_CONTENT" "考试" "automation exam calendar keyword"
...
check_contains "$README_CONTENT" "自动化关键词目录" "automation keyword catalog docs"
...
echo "==> Verifying SwiftUI parity plan"
SWIFTUI_PLAN_OUTPUT="$(scripts/swiftui_parity_plan.sh --strict)"
[[ "$SWIFTUI_PLAN_OUTPUT" == *"Readiness:"*"SwiftUI parity plan assessed"* ]] || fail "SwiftUI parity plan did not complete"
[[ "$SWIFTUI_PLAN_OUTPUT" == *"Next phase:"*"settings-contract-foundation"* ]] || fail "SwiftUI parity plan next phase mismatch"
...
echo "==> Verifying automation keyword config readiness"
AUTOMATION_KEYWORD_CONFIG_OUTPUT="$(scripts/automation_keyword_config_readiness.sh --strict)"
[[ "$AUTOMATION_KEYWORD_CONFIG_OUTPUT" == *"Readiness:"*"automation keyword config assessed"* ]] || fail "automation keyword config readiness did not complete"
[[ "$AUTOMATION_KEYWORD_CONFIG_OUTPUT" == *"Template Coverage"* ]] || fail "automation keyword config readiness missing template coverage"
